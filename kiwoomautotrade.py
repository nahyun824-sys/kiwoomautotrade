...
class Kiwoom(QAxWidget):
    def __init__(self):
        ...
        # ✅ 기존보유 손절 판단용 손익률 맵
        self.balance_profit_rates = {}  # {code: profit_rate_float}
        ...
        # 최근 매수 시도 쿨다운
        self.last_buy_attempt_ts = {}
        self.BUY_COOLDOWN_SEC = 180

        # 손절
        self.STOPLOSS_RATE = -10.0

        # ✅ (추가) opt10001 레이트리밋 (TR 과부하 ret=-200 완화)
        self.last_price_req_ts = 0.0
        self.PRICE_REQ_INTERVAL = 0.40  # 0.35~0.60 권장
    ...
    # ✅ (수정 1) opw00018 파싱: GetCommData + 종목번호(안정) + 손익율/수익률 fallback
    ...
    def enqueue_buy(self, code, amount, buy_cond_index=None, buy_cond_name=""):
        ...
        if now - last_ts < self.BUY_COOLDOWN_SEC:
            remain = self.BUY_COOLDOWN_SEC - (now - last_ts)
            print(f"[BUY-QUEUE] 최근 매수시도({self.BUY_COOLDOWN_SEC}s) → 스킵: {code}, 남은 {remain:.1f}s")
            return
    ...
    def request_price(self, code):
        ...
        if gap < self.PRICE_REQ_INTERVAL:
            time.sleep(self.PRICE_REQ_INTERVAL - gap)
    ...
    # 체잔 수신
    # -----------------------------
    # ✅ (수정 3) pending 해제 강화: 코드 정규화 + gubun 0/1 모두에서 안정적으로 pop
